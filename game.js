...
function renderChoices(s){const box=$("#choices");box.innerHTML="";s.choices.slice(0,innerWidth<500?3:4).forEach((c,i)=>{const locked=(state.attention>=3&&c[2].type==="rebel")||(c[2].need&&!state.flags[c[2].need])||(c[2].needAl&&state.alberto<c[2].needAl);const b=document.createElement("button");b.className="choice"+(locked?" locked":"");b.innerHTML=`<b>${i+1}. ${c[0]}</b><small>${locked?lockReason(c[2]):c[1]}</small>`;b.disabled=locked;b.onclick=()=>choose(i);box.appendChild(b)})}
